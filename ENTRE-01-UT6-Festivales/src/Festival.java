...
public class Festival {
    ...
    public boolean haConcluido() {
        ...
        return LocalDate.now().isAfter(fechaInicio.plusDays(duracion));
    ...
    public String toString() {
        int diasRestantes = 0;
        String dR;
        if(LocalDate.now().getYear() - fechaInicio.getYear() < 0){
            diasRestantes = 0;
            dR = "(ON)";
        }else{
            diasRestantes = (LocalDate.now().getDayOfYear() - fechaInicio.getDayOfYear());
            dR = "(quedan " + diasRestantes + " d�as)";
        }
